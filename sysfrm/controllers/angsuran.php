...
switch ($action) {
    case 'add':
        ...
        $ui->assign('xfooter', Asset::js(array('numeric','s2/js/select2.min','dp/dist/datepicker.min','s2/js/i18n/'.lan(),'add-pesanan','modal','btn-top/btn-top')));
        $ui->assign('xjq', '
 ...
 ');
    ...
    case 'edit':
        ...
        if ($d) {
			...
			$ui->assign('xheader', Asset::css(array('s2/css/select2.min','dp/dist/datepicker.min','modal','btn-top/btn-top')));
			$ui->assign('xfooter', Asset::js(array('numeric','s2/js/select2.min','dp/dist/datepicker.min','s2/js/i18n/'.lan(),'edit-pesanan','modal','btn-top/btn-top')));
			$ui->assign('xjq', '
				 $(\'.amount\').autoNumeric(\'init\');
				 ');
    ...
    case 'view':
        ...
        if ($d) {
			...
			$ui->assign('idate', date('d-m-Y',strtotime($d['tgl_pesan'])));
			$ui->assign('xheader', Asset::css(array('s2/css/select2.min','dp/dist/datepicker.min','modal','btn-top/btn-top')));
			$ui->assign('xfooter', Asset::js(array('numeric','s2/js/select2.min','dp/dist/datepicker.min','s2/js/i18n/'.lan(),'view-pesanan','modal','btn-top/btn-top')));
			$ui->assign('xjq', '
				 $(\'.amount\').autoNumeric(\'init\');
				 ');
...
//			echo $msg;
    ...
    case 'list':
        ...
        //find all tags
        if($name != ''){
			$paginator = Paginator1::bootstrap('data_pesanan','ORM::for_table(data_pesanan)->table_alias("a")->select("a.*")->select("b.account")->join("crm_accounts",array("a.id_cust","=","b.id"),"b")->where_like("'.$filter.'","%"'.$name.'"%")->order_by_asc("no_jual")->order_by_desc("tgl_pesan")->order_by_desc("no_pesan")->find_many()',20);
			$d=ORM::for_table(data_pesanan)->table_alias("a")->select("a.*")->select("b.account")->select("b.address")->select("b.phone")->join("crm_accounts",array("a.id_cust","=","b.id"),"b")->where_like($filter,'%'.$name.'%')->offset($paginator['startpoint'])->limit($paginator['limit'])->order_by_asc('no_jual')->order_by_desc("tgl_pesan")->order_by_desc("no_pesan")->find_many();
			$ui->assign('cari',$name);
			if($filter == 'no_pesan') {
			...
			$paginator = Paginator1::bootstrap('data_pesanan','ORM::for_table(data_pesanan)->table_alias("a")->select("a.*")->select("b.account")->join("crm_accounts",array("a.id_cust","=","b.id"),"b")->order_by_asc("no_jual")->order_by_desc("tgl_pesan")->order_by_desc("no_pesan")->find_many()',20);
			$d=ORM::for_table(data_pesanan)->table_alias("a")->select("a.*")->select("b.account")->select("b.address")->select("b.phone")->join("crm_accounts",array("a.id_cust","=","b.id"),"b")->offset($paginator['startpoint'])->limit($paginator['limit'])->order_by_asc('no_jual')->order_by_desc("tgl_pesan")->order_by_desc("no_pesan")->find_many();
		}
        ...
        $ui->assign('xfooter', Asset::js(array('btn-top/btn-top','list-pesanan'))
);
        $ui->assign('jsvar', '
_L[\'are_you_sure\'] = \''.$_L['are_you_sure'].'\';
 ');
        $ui->display('list-pesanan.tpl');
    ...
    case 'delete':
        ...
        if($d){
			if($d['bayar'] > 0 or $d['no_jual']<> '') {
				r2(U.$myCtrl.'/list/', 'e', 'Telah ada transaksi pada pesanan tersebut');
			} else {
				$d->delete();
				_log1('Deleted Pesanan: '.$id,$user['username'],$user['id']);
				r2(U.$myCtrl.'/list/', 's', 'Pesanan Berhasil di hapus');
			}
        } else
			r2(U.$myCtrl.'/list/','e',$id);
