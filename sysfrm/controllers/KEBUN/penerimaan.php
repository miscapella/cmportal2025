...
switch ($action) {
    case 'list':
		...
		$msg = $routes['3'];
        $d = ORM::for_table('spbi_master')->where('status', 'DITERIMA')->where('kepada',$user['username'])->order_by_desc('no_bpnb')->find_many();
        $cd = ORM::for_table('spbi_master')->where('status', 'DITERIMA')->order_by_desc('no_bpnb')->count();
        ...
        $ui->assign('xfooter', Asset::js(array('s2/js/select2.min','s2/js/i18n/'.lan(),$spath.'list-bpnb','dp/dist/datepicker.min','btn-top/btn-top','numeric')));
    ...
    case 'add':
        ...
        $tg = ORM::for_table('spbi_master')->where('status','DIKIRIM')->where('kepada', $user['username'])->find_many();
        foreach ($tg as $r) {
    ...
    case 'add-post':
        ...
        if($file_bpnb == ''){
            $msg .= 'Bukti penerimaan tidak boleh kosong<br>';
        }
        if($msg == ''){
            ...
            try {
                ...
                foreach($no_pr as $code) {
                    ...
                    $y = ORM::for_table('pr_detail')->where('no_pr', $snopr)->where('kode_item', $skodeitem)->find_one();
                    $y->status = 'DITERIMA';
                    $y->save();
                    $i++;
                }
    ...
    case 'render-bpnb':
     ...
     //add by Ramiro 12 Mar 2024 16:10
    case 'upload-file':
