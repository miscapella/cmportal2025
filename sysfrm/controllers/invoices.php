...
//find the user
            ...
            $ui->assign('a', $a);
            $ui->assign('d', $d);
            $ui->assign('_st', 'Edit Faktur Penjualan');
...
//        $its = strtotime($idate);
		...
		foreach($qty as $value) {
			$sbatch=$batch[$i];
			$a = ORM::for_table('sys_stock')->where('batch_number',$sbatch)->find_one();
			if($a) {
				if(intval($a['item_number'])<intval($value))
					$msg .= 'Batch No. '.$sbatch.' tidak mencukupi ! Sisa : '.$a['item_number'].'<br>';
			}
			else
				$msg .= $sbatch.' tidak ditemukan !<br>';
...
//                }
            ...
            //Create No. Faktur Jual
			$bl=date('n',strtotime(_post('idate')));
			$th=date('Y',strtotime(_post('idate')));
			$query = ORM::for_table('sys_invoices')->raw_query("SELECT * FROM sys_invoices where month(date)=$bl and year(date)=$th order by invoicenum desc")->find_one();
			if($query) {
				$invoicenum = ++$query['invoicenum'];
			} else {
				$invoicenum = 'PJL/'.date('m',strtotime(_post('idate'))).'/'.date('Y',strtotime(_post('idate'))).'/0001';
			}
...
//                }
				...
				//Update Stock
 				$z = ORM::for_table('sys_stock')->where('code',$scode)->where('batch_number',$sbatch)->find_one();
				if($z) {
...
//        $its = strtotime($idate);
		$idate = date('Y-m-d', strtotime(_post('idate')));
        $its = strtotime($idate);
        $duedate = date('Y-m-d', strtotime(_post('ddate')));
        $repeat = _post('repeat');
        $nd = $idate;
        if ($repeat == '0') {
            $r = '0';
...
//                }
            ...
            $d->nd = $nd;
				...
				//kembalikan stock
				$b = ORM::for_table('sys_invoiceitems')->where('invoiceid',$iid)->find_many();
				foreach ($b as $acs) {
...
//find all clients.
        ...
        $ui->assign('_st', 'Penerimaan Piutang');
		$c = ORM::for_table('sys_invoices')->select('id')->select('invoicenum')->select('account')->order_by_asc('date')->find_many();
        $ui->assign('c', $c);
 ...
 '.
    ...
    case 'render-target':
        ...
        Event::trigger('invoices/render-target/');

        $cid = _post('cid');
        $a = ORM::for_table('sys_invoices')->where('id',$cid)->find_one();
		if($a) {
			$data = array(
					'ketkw'		=>  'Pembayaran Penjualan, INV : #'.$a['invoicenum'],
...
<div class="form-group">
    ...
    <div class="col-sm-10">
      <input type="text" id="description" name="description" class="form-control" value="Pembayaran Penjualan, INV : #' . $d['invoicenum'] . '">
    </div>
  </div>
<div class="form-group">
