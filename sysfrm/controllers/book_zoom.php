...
function delete_meeting($meeting_id) {
    ...
    try {
        ...
        if (204 == $response->getStatusCode()) {
            return true; // Meeting berhasil dihapus
        }
        return false;
        ...
        if (401 == $e->getCode()) {
            ...
            // Coba delete meeting lagi dengan token baru
            return delete_meeting($meeting_id);
        } else {
            throw $e; // Re-throw exception untuk error selain 401
        }
    }
}
...
function createMeeting($data = array()) {
    ...
    try {
        $response = $client->request('POST', '/v2/users/capella.zoom@gmail.com/meetings', [
            "headers" => [
                "Authorization" => "Bearer $accessToken"
            ],
            'json' => [
        ...
        if(!empty($result)) {
            $hasil['topic'] = $result->topic;
            $hasil['zoom_url'] = $result->join_url;
            $hasil['meeting_id'] = $result->id;
            $hasil['passcode_zoom'] = $result->password;
            $hasil['start_zoom'] = $result->start_time;
            $hasil['zoom_duration'] = $result->duration;
        }
        return $hasil;
        ...
        if (401 == $e->getCode()) {
            ...
            // Coba create meeting lagi dengan token baru
...
function cekWaktu($tanggal_waktu, $durasi) {
    ...
    $d = ORM::for_table('daftar_booking_zoom','dblogin')->where_not_in('subjek',['Pinjam Barang Inventaris', 'Pinjam Ruangan'])->where_in('status', array('PEND', 'READY'))->find_many();
    foreach($d as $ds) {
...
//        echo "<script type='text/javascript'>alert('$message');</script>";
        ...
        $d = ORM::for_table('daftar_booking_zoom','dblogin')->where_not_in('subjek', ['Pinjam Barang Inventaris', 'Pinjam Ruangan'])->where_in('status', array('PEND', 'READY'))->order_by_asc('tanggal_meeting')->find_many();
...
//        $ui->assign('e',$e);
    ...
    case 'add-post':
        ...
        $selected_date = date('Y-m-d', strtotime($tanggal_meeting));
        if ($selected_date < date('Y-m-d')) {
            $msg .= 'Tanggal meeting tidak boleh sebelum hari ini <br>';
        }
        if($durasi == ''){
        ...
        if($msg == ''){
			...
			try {
                ...
                if(isset($result['meeting_id'])){
                    $zoom_url=$result['zoom_url'];
                    $meeting_id=$result['meeting_id'];
                    $passcode_zoom=$result['passcode_zoom'];
                    $start_zoom=$result['start_zoom'];
                    $zoom_duration=$result['zoom_duration'];
    ...
    case 'addPinjaman-post':
        ...
        $selected_date = date('Y-m-d', strtotime($tanggal_meeting));
        if ($selected_date < date('Y-m-d')) {
            $msg .= 'Tanggal meeting/pinjaman tidak boleh sebelum hari ini <br>';
        }
