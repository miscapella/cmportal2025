...
switch ($action) {
case 'list-approval':
	    $columns = array('a.id','a.add_date','a.kode_form','a.nama_form','a.fullname');
		...
		$sql = "SELECT
            ...
            b.nama_form,
            a.fullname,
            SUBSTRING_INDEX(SUBSTRING_INDEX(a.approve_key, ',', FIND_IN_SET('$user_name', approval_by)), ',', -1) AS approve_key,
            a.approval_by,
            a.approval_status,
            SUBSTRING_INDEX(SUBSTRING_INDEX(a.reject_key, ',', FIND_IN_SET('$user_name', approval_by)), ',', -1) AS reject_key,
            SUBSTRING_INDEX(SUBSTRING_INDEX(a.comment_key, ',', FIND_IN_SET('$user_name', approval_by)), ',', -1) AS comment_key
        FROM
            daftar_response a
        INNER JOIN
            form_master b ON a.kode_form = b.kode_form
        WHERE
            a.status = 'In Progress'
            AND FIND_IN_SET('In Progress', approval_status) = FIND_IN_SET('$user_name', approval_by)
            AND FIND_IN_SET('$user_name', approval_by) > 0";
		...
		'[fullname]',
	        ...
	        <a onclick="return confirm('. $approve_msg .')" href="'. $url .'?ng=form-api/approve-form/[id]/token_[approve_key]" target="_blank" class="btn btn-success btn-xs">Approve</a>
	        <a onclick="return confirm('.$reject_msg.')" href="'.$url.'?ng=form-api/reject-form/[id]/token_[reject_key]" target="_blank" class="btn btn-danger btn-xs">Reject</a>
	        <a <a href="'.$url.'?ng=form-api/comment-form/[id]/token_[comment_key]" target="_blank" class="btn btn-primary btn-xs">With Comment</a></div></td>'
	);
	loadTable($conn, $columns, $sql, $whereArr, $show);
	// $columns = array('a.id', 'a.add_date', 'a.kode_form', 'a.nama_form', 'a.fullname');
	// $whereArr = array('b.nama_form', 'a.fullname');
	// $user_name = $user['username'];
	// $sql = "SELECT a.id, a.add_date, a.kode_form, b.nama_form, a.fullname, a.approve_key, a.approval_by,
	// a.approval_status, a.reject_key, a.comment_key FROM daftar_response a inner join form_master b on a.kode_form=b.kode_form where a.status='In Progress'";
	...
	// 		if ($item == $whereArr[0]) {
	// 			$where .= $item . " LIKE '%" . $params['search']['value'] . "%' ";
	// 		} else {
	// 			$where .= " OR " . $item . " LIKE '%" . $params['search']['value'] . "%' ";
	// 		}
	...
	// 		if ($itemby == $user['username'] && $approval_status[$nokey] == 'In Progress' && $approval_status[$nokey - 1] != 'In Progress') {
	// 			$data1[] = $nourut;
	// 			$data1[] = '<td style="text-align: center; vertical-align: middle;white-space: nowrap;">' . date('d-m-Y', strtotime($row['add_date'])) . '</td>';
...
case 'load-detail-response':
	...
	$show = array(
		...
		'<div class="text-center">
                <a class="detail" value="[id]"><i class="fa fa-bar-chart-o"></i>Details</a>
				<a href="[url]form/print-approve/[id]/" target="_blank" class="btn btn-primary btn-xs" id="uid[id]"><i class="fa fa-print"></i> Print</a>
            </div>',
	);
	loadTable($conn, $columns, $sql, $whereArr, $show);
	break;
...
case 'history-input':
	...
	$columns = array('add_date', 'kode_form', 'status','manage');
	$whereArr = array('add_date', 'kode_form', 'status', 'manage');
	// $sql = "SELECT id, add_date, kode_form,  status FROM daftar_response";
	$sql = "
...
case 'list-response-detail':
	...
	while ($row = mysqli_fetch_array($queryRecords)) {
		...
		foreach ($jawaban as $item) {
			$link = explode("http://192.168.201.180/cmportal/uploads/FORM/", $item);
			if (count($link) == 2) {
				$pisah = str_replace("http://192.168.201.180/cmportal/uploads/FORM/", "", $item);
				$data1[] = '<td style="text-align: center; vertical-align: middle;white-space: nowrap;"><a href="http://192.168.201.180/cmportal/uploads/FORM/' . $pisah . '" target="_blank">' . $pisah . '</a></td>';
			} else {
