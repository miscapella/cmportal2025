...
Class Paginator
{
    public static function bootstrap($table, $w1='',$c1='', $w2='', $c2= '', $w3='',$c3='', $w4='', $c4= '', $per_page = '50',$dbconn = '')
    {
global $routes;
        global $_L;
        $txt_next = $_L['Next'];
...
//        }
        ...
        elseif($w2 != ''){
			...
			if($dbconn != '')
				$totalReq = ORM::for_table($table,$dbconn)->where_raw('(`'.$w1.'` = ? OR `'.$w2.'` = ?)', array($c1, $c2))->count();
			else
				$totalReq = ORM::for_table($table)->where_raw('(`'.$w1.'` = ? OR `'.$w2.'` = ?)', array($c1, $c2))->count();
        }
