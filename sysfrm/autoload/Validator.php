...
class Validator{
    ...
    public static function FixLength($string, $len=3){
        $length = strlen($string);
    ...
    public static function Date1($string){
        $date = date('Y', strtotime($string));
		$date1 = date('Y');
		if($date >= $date1-100)
			if(checkdate(substr($string,3,2),substr($string,0,2),substr($string,6))) {
				return date('Y-m-d',strtotime(substr($string,6)."-".substr($string,3,2)."-".substr($string,0,2)));
			}
			else
				return 'Salah';
		else
