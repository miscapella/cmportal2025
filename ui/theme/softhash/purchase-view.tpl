    ...
    <div class="col-lg-12"  id="sysfrm_ajaxrender">
        <div class="ibox float-e-margins">
            <div class="ibox-title">
                <h5>FAKTUR BELI - {$d['invoicenum']}</h5>
...
{*
                    ...
                    <a href="{$_url}purchase/edit/{$d['id']}" class="btn btn-warning  btn-sm"><i class="fa fa-pencil"></i> {$_L['Edit']}</a>
                    <div class="btn-group" role="group">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            <li><a href="{$_url}purchase/pdf/{$d['id']}/view" target="_blank">{$_L['View PDF']}</a></li>
                            <li><a href="{$_url}purchase/pdf/{$d['id']}/dl">{$_L['Download PDF']}</a></li>
                        </ul>
                    </div>
                    ...
                    <header class="clearfix">
                        <div class="row">
                            <div class="col-sm-6 mt-md">
                                <h2 class="h2 mt-none mb-sm text-dark text-bold">FAKTUR BELI</h2>
                    ...
                    <div class="table-responsive">
                        <table class="table invoice-items">
                            ...
                            <tr class="h4 text-dark">
                                ...
                                <th id="cell-batch" class="text-semibold">Batch Number</th>
                            ...
                            {foreach $items as $item}
                                <tr>
                                    ...
                                    <td class="text-semibold text-dark">{$item['batch_number']}</td>
                                    <td class="text-right">{$_c['currency_code']} {number_format($item['amount'],2,$_c['dec_point'],$_c['thousands_sep'])}</td>
                                    <td class="text-right">{$item['qty']}</td>
                                    <td class="text-right">{$_c['currency_code']} {number_format($item['total'],2,$_c['dec_point'],$_c['thousands_sep'])}</td>
                                </tr>
                            {/foreach}
                            ...
                            <div class="col-sm-4 col-sm-offset-8">
                                <table class="table h5 text-dark">
                                    ...
                                    <tr class="b-top-none">
                                        ...
                                        <td class="text-right">{$_c['currency_code']} {number_format($d['subtotal'],2,$_c['dec_point'],$_c['thousands_sep'])}</td>
                                    </tr>
                                   {if ($d['discount']) neq '0.00'}
                                       <tr>
                                           <td colspan="2">{$_L['Discount']}</td>
                                           <td class="text-right">{$_c['currency_code']} {number_format($d['discount'],2,$_c['dec_point'],$_c['thousands_sep'])}</td>
                                       </tr>
                                   {/if}
                                    <tr>
                                        <td colspan="2">{$_L['TAX']}</td>
                                        <td class="text-right">{$_c['currency_code']} {number_format($d['tax'],2,$_c['dec_point'],$_c['thousands_sep'])}</td>
                                    </tr>
                                    {if ($d['credit']) neq '0.00'}
                                        <tr>
                                            <td colspan="2">{$_L['Total']}</td>
                                            <td class="text-right">{$_c['currency_code']} {number_format($d['total'],2,$_c['dec_point'],$_c['thousands_sep'])}</td>
                                        </tr>
                                        <tr>
                                            <td colspan="2">{$_L['Total Paid']}</td>
                                            <td class="text-right">{$_c['currency_code']} {number_format($d['credit'],2,$_c['dec_point'],$_c['thousands_sep'])}</td>
                                        </tr>
                                        <tr class="h4">
                                            <td colspan="2">{$_L['Amount Due']}</td>
                                            {*<td class="text-right">{$_c['currency_code']} {number_format($i_due,2,$_c['dec_point'],$_c['thousands_sep'])}</td>*}
                                            <td class="text-right">{$_c['currency_code']} {$i_due}</td>
                                        </tr>
                                        {else}
                                        <tr class="h4">
