		...
		<div class="panel panel-default">
			<div class="panel-body">
                ...
                <div class="ibox-tools">
					<a href="{$_url}pembelian/list-pr-pending/" class="btn btn-primary btn-sm">Daftar PR</a>
				</div>
							...
							<tr>
								...
								<td>
									<select name="kd_inventaris[]" class="kd_inventaris" id="kd_inventaris" disabled>
										...
										{/foreach} -->
										<option value="{$ds['keperluan']}" selected>{($ds['keperluan'] == 'PENGADAAN') ? 'PENGADAAN BARU' : $ds['keperluan']}</option>
									</select>
									<!-- <input type="text" style="background-color: #ccc" name="{$ds['keperluan']}" class="{$ds['keperluan']}" readonly value="{$ds['keperluan']}"> -->
								</td>
								<td>
									<select name="kd_item[]" class="kd_item" id="kd_item" class="kd_item" disabled> -->
										{foreach $tg1 as $r1}
										{if $ds['kd_item'] eq $r1['kd_item']}
											<option value="{$r1['kd_item']}" selected>{$r1['nm_item']}</option>
										{/if}
									{/foreach}
									...
									<!-- {foreach $tg1 as $r1}
										{if $ds['kd_item'] eq $r1['kd_item']}
											<input type="text" style="background-color: #ccc" name="{$r1['kd_item']}" class="{$r1['kd_item']}" value="{$r1['nm_item']}" readonly>
										{/if}
									{/foreach} -->
								</td>
								...
								<td><input type="text" style="cursor: pointer; background: white;" name="tgl[]" class="tgl" datepicker="" data-date-format="dd-mm-yyyy" data-auto-close="true" value={if $ds['tgl_diperlukan'] neq ''}{date('d-m-Y', strtotime($ds['tgl_diperlukan']))}{else}""{/if}></td>
