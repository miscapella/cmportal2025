                ...
                <table class="table table-bordered table-hover sys_table">
                    ...
                    {foreach $d as $ds}
                        <tr {if $ds['priority'] eq 'URGENT'}style="background-color:#ffc9bb;"{/if}>
                            <td>{$nourut} </td>
                            <td>{$ds['no_spmk']}</td>
                            <td>{date( $_c['df'], strtotime($ds['tgl_spmk']))}</td>
                            <td>{$ds['dibuat_nama']}</td>
                            <td>{$ds['priority']}</td>
                            <td>{$ds['status']}</td>
                            <td class="text-right">
                                <a href="{$_url}pembelian/detail-spmk-aprv/{$ds['id']}/" class="btn btn-success btn-xs"><i class="fa fa-pencil-square-o"></i> Detail Approval</a>
                ...
                <table class="table table-bordered table-hover sys_table">
                    ...
                    {foreach $e as $es}
                        <tr {if $es['priority'] eq 'URGENT'}style="background-color:#ffc9bb;"{/if}>
                            <td>{$nourut} </td>
                            <td>{$es['no_spmk']}</td>
                            <td>{date( $_c['df'], strtotime($es['tgl_spmk']))}</td>
                            <td>{$es['dibuat_nama']}</td>
                            <td>{$es['priority']}</td>
                            <td>{$es['status']}</td>
                            <td class="text-right">
                                <a href="{$_url}pembelian/detail-spmk-aprv/{$es['id']}/" class="btn btn-success btn-xs"><i class="fa fa-pencil-square-o"></i> Detail Revisi</a>
                            </td>
                        </tr>
                        {assign var="nourut" value=$nourut+1}
                    {/foreach}
                ...
                <table class="table table-bordered table-hover sys_table">
                    ...
                    {foreach $f as $fs}
                        <tr {if $fs['priority'] eq 'URGENT'}style="background-color:#ffc9bb;"{/if}>
                            <td>{$nourut} </td>
                            <td>{$fs['no_spmk']}</td>
                            <td>{date( $_c['df'], strtotime($fs['tgl_spmk']))}</td>
                            <td>{$fs['dibuat_nama']}</td>
                            <td>{$fs['priority']}</td>
                            <td>{$fs['status']}</td>
                            <td class="text-right">
                                <a href="{$_url}pembelian/detail-spmk-aprvsup/{$fs['id']}/" class="btn btn-success btn-xs"><i class="fa fa-pencil-square-o"></i> Detail Approval</a>
                            </td>
                        </tr>
                        {assign var="nourut" value=$nourut+1}
                    {/foreach}
                ...
                <table class="table table-bordered table-hover sys_table">
                    ...
                    {foreach $g as $gs}
                        <tr {if $gs['priority'] eq 'URGENT'}style="background-color:#ffc9bb;"{/if}>
                            <td>{$nourut} </td>
                            <td>{$gs['no_spmk']}</td>
                            <td>{date( $_c['df'], strtotime($gs['tgl_spmk']))}</td>
                            <td>{$gs['dibuat_nama']}</td>
                            <td>{$gs['priority']}</td>
                            <td>{$gs['status']}</td>
                            <td class="text-right">
                                <a href="{$_url}pembelian/detail-spmk-aprvsup/{$gs['id']}/" class="btn btn-success btn-xs"><i class="fa fa-pencil-square-o"></i> Detail Revisi</a>
                            </td>
                        </tr>
                        {assign var="nourut" value=$nourut+1}
                    {/foreach}
