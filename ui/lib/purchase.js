...
//	$('.item_price').blur(function(){
    ...
    $('#contact_add').on('click', function(e){
        ...
        setTimeout(function(){
            $modal.load('?ng=supplier/modal_add/', '', function(){
                $modal.modal();
	...
	$('#invoice_items').on('change', '.batch', function(){
	   //  Cek Dobel
		var item_check = $('input[name="batch[]"]').map(function(){return $(this).val();}).get();
		var ssitem = 0;
		for (var i = 0; i < item_check.length; i++) {
			if (item_check[i] === $(this).val()) {
				ssitem++;
			}
		}
		if (ssitem>1) {
			alert("No. Batch telah ada !");
			$(this).val("");
			$(this).focus();
		}
	});
...
//                var batch_number = $(this).closest('tr').find('td:eq(3)').text();
				var batch_number="";
                    ...
                    setTimeout(function () {
                        ...
                        if ($.isNumeric(data)) {
                           ...
                           // location.reload();
                            ...
                            if(is_recurring == 'yes'){
                                window.location = _url + 'purchase/add/recurring/' + data + '/';
                            }
                            else{
                                window.location = _url + 'purchase/add/1/' + data + '/';
                            }
    ...
    $("#submit").click(function (e) {
        ...
        var _url = $("#_url").val();
        $.post(_url + 'purchase/add-post/', $('#invform').serialize(), function(data){

                setTimeout(function () {
                    var sbutton = $("#submit");
                    var _url = $("#_url").val();
                    if ($.isNumeric(data)) {
						alert('Pembelian telah berhasil disimpan');
						window.location = _url + 'purchase/list/';
                        //window.location = _url + 'purchase/view/' + data + '/';
                    }
                    else {
                        $('#ibox_form').unblock();
                        var body = $("html, body");
