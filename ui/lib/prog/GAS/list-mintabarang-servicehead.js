$(document).ready(function () {
    ...
    $.ajax({
        url: _url + "serverside/list-mintabarang-servicehead/",
        type: 'POST',
        dataType: 'json',
    ...
    function loadTable() {
        $('#list-mintabarang').DataTable({
            ...
            "createdRow": function(row, data, dataIndex) {
                ...
                else if (data[5] == "CANCEL")
                    $('td:eq(5)', row).html(`<div class="text-center"><span class="status btn btn-danger btn-xs">CANCELLED</span></div>`);
                else
                    $('td:eq(5)', row).html(`<div class="text-center"><span class="status btn btn-primary btn-xs">${data[5]}</span></div>`);

                $('td:eq(6)', row).html(`<div class="text-right">
                    <a class="btn btn-success btn-sm cdetail" id="uid${data[6]}" title="Detail"><i class="fa fa-book"></i></a>
                </div>`);
            },
            ...
            "initComplete": function() {
                ...
                $('#status-filter').select2({
                    ...
                    table.column(5).header().style.pointerEvents = 'none';
                }).on('select2:close', function(e) {
                    table.column(5).header().style.pointerEvents = '';
                }).on('change', function() {
                    ...
                    if (filter == "") {
                        ...
                        filteredTableData = tableData.filter(data => data[5] == filter);
                    }
