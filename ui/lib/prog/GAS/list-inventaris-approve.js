$(document).ready(function () {
	...
	$("#save").click(function (e) {
		...
		$.each($("input[name='chk[]']:checked"), function(){
			var kode = $(this).closest('tr').find("input[name='kd_item[]']").val();
            var kode1 = $(this).closest('tr').find("input[name='kd_inventaris[]']").val();
			kd_inventaris.push($(this).closest('tr').find("input[name='kd_inventaris[]']").val());
		...
		$.ajax({
			url: _url + 'inventaris/approve-post/',
			type: 'POST',
			data: $data,
			cache: false,
			//dataType: 'json',
