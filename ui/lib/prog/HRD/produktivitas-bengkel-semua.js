$(document).ready(function () {
	...
	semuaCabang.forEach(({ id }) => {
		$.ajax({
			...
			processData: false,
			success: function() {
				if (++ loaded === semuaCabang.length) setupGrafik('grafik_unit_entry_semua');
			},
	...
	$('[id="grafik_unit_entry_semua-tipe"]').change(function () {
		const grafik = $(this).attr('id').split('-')[0];
		setupGrafik(grafik);
	});

	$('[id="grafik_unit_entry_semua-dari"], [id="grafik_unit_entry_semua-hingga"], [id="grafik_unit_entry_semua-cat"]').change(function () {
		const grafik = $(this).attr('id').split('-')[0];
		loadGrafik(grafik);
	});
	...
	function loadGrafik(gid) {
		...
		const category = $(grafikId + '-cat').val();
