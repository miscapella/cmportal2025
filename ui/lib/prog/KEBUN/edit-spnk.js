$(document).ready(function () {
	...
	$("#save").click(function (e) {
		...
		$.each($("input[name='chk[]']:checked"), function(){
            var kode = $(this).closest('tr').find("input[name='kontraktorid[]']").val();
            ...
            kontraktor1.push($(this).closest('tr').find("select[name='kontraktor1[]'] option").filter(':selected').val());
            harga1.push($(this).closest('tr').find("input[name='harga1[]']").val());
			keterangan_kontraktor1.push($(this).closest('tr').find("input[name='keterangan_kontraktor1[]']").val());
			file_kontraktor1.push($(this).closest('tr').find("input[name='file_kontraktor1[]']").val());
            kontraktor2.push($(this).closest('tr').find("select[name='kontraktor2[]'] option").filter(':selected').val());
            harga2.push($(this).closest('tr').find("input[name='harga2[]']").val());
			keterangan_kontraktor2.push($(this).closest('tr').find("input[name='keterangan_kontraktor2[]']").val());
			file_kontraktor2.push($(this).closest('tr').find("input[name='file_kontraktor2[]']").val());
            kontraktor3.push($(this).closest('tr').find("select[name='kontraktor3[]'] option").filter(':selected').val());
            harga3.push($(this).closest('tr').find("input[name='harga3[]']").val());
			keterangan_kontraktor3.push($(this).closest('tr').find("input[name='keterangan_kontraktor3[]']").val());
			file_kontraktor3.push($(this).closest('tr').find("input[name='file_kontraktor3[]']").val());
            kontraktorpilihan.push($(this).closest('tr').find("input[name='"+kode+"kontraktorpilihan[]']:checked").val());
		});
		...
		$data.append('ha', ha);
		$data.append('pkk', pkk);
        $data.append('kontraktor1', kontraktor1);
        ...
        $data.append('kontraktor2', kontraktor2);
        ...
        $data.append('kontraktor3', kontraktor3);
		...
		bootbox.confirm('Apakah anda yakin untuk mengubah SPnK ?', function(result) {
			if(result){
				bootbox.dialog({
					message: '<div class="text-center"><i class="fas fa-spinfa-spinner"></i> Loading...</div>',
					closeButton: false
				...
				$.ajax({
					url: _url + 'pembelian/edit-spnk-post/',
					type: 'POST',
					data: $data,
					cache: false,
					processData: false, 
					...
					success: function(data, textStatus, jqXHR)
					{
						...
						if ($.isNumeric(obj.dataval)) {
							...
							bootbox.alert({
								...
								timeout : 2000,
								callback: function(){ 
									window.location = _url + 'pembelian/list-spnk-pending/SPnK Berhasil Ditambah';
								}
							});
						}
						else {
