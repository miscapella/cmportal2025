$(document).ready(function () {
	...
	$("#save").click(function (e) {
		...
		$.each($("input[name='chk[]']:checked"), function(){
            ...
            kode_supplier1.push($(this).closest('tr').find("select[name='kode_supplier1[]'] option").filter(':selected').val());
            harga1.push($(this).closest('tr').find("input[name='harga1[]']").val());
			keterangan_supplier1.push($(this).closest('tr').find("input[name='keterangan_supplier1[]']").val());
			file_supplier1.push($(this).closest('tr').find("input[name='file_supplier1[]']").val());
            kode_supplier2.push($(this).closest('tr').find("select[name='kode_supplier2[]'] option").filter(':selected').val());
            harga2.push($(this).closest('tr').find("input[name='harga2[]']").val());
			keterangan_supplier2.push($(this).closest('tr').find("input[name='keterangan_supplier2[]']").val());
			file_supplier2.push($(this).closest('tr').find("input[name='file_supplier2[]']").val());
            kode_supplier3.push($(this).closest('tr').find("select[name='kode_supplier3[]'] option").filter(':selected').val());
            harga3.push($(this).closest('tr').find("input[name='harga3[]']").val());
			keterangan_supplier3.push($(this).closest('tr').find("input[name='keterangan_supplier3[]']").val());
			file_supplier3.push($(this).closest('tr').find("input[name='file_supplier3[]']").val());
            supplierpilihan.push($(this).closest('tr').find("input[name='"+kode1+kode+"supplierpilihan[]']:checked").val());
		});
		...
		bootbox.confirm('Apakah anda yakin untuk mengubah PR ?', function(result) {
			if(result){
				...
				$.ajax({
					url: _url + 'pembelian/edit-pr-supplier-post/',
					type: 'POST',
					data: $data,
					cache: false,
					processData: false,
					...
					success: function(data, textStatus, jqXHR)
					{
						...
						if ($.isNumeric(obj.dataval)) {
							...
							bootbox.alert({
								...
								timeout : 2000,
								callback: function(){ 
									window.location = _url + 'pembelian/list-pr1-pending/PR Biding Berhasil Diubah';
								}
							});
						}
						else {
